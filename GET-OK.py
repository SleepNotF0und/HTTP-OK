...
print(bcolors['BOLD'] +"                                        Coded By hackerone.com/0xr3dhunt V2 \n\n" + bcolors['ENDC'])
...
def Make_Requests():   
    ...
    if inpt not in ['Y','y']: 
        ...
        for line in PerfectLines:
            try:
                link = 'https://'+line
                req = requests.get(link, verify=False, allow_redirects=False, timeout=5)
                ...
                Make_Requests.DEADSUBS.append(link.ljust(55) + "           =========>  " + "[Failed Time Out]")
